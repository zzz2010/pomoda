...
private:
    ...
    int i,j,k,l;
public:
    IntMatrix(int x, std::string name="");
    IntMatrix(int x, int y, std::string name="");
    IntMatrix(int x, int y, int z, std::string name="");
    IntMatrix(int x, int y, int z, int w, std::string name="");
    ...
    void s(int v, int xa, int ya=0, int za = 0, int wa = 0) { /* std::cout<<name<<" "<<v<<" "<<xa<<" "<<ya<<" "<<za<<" "<<wa<<std::endl;/**/ assert(xa>=0); assert(xa<x); assert(ya>=0); assert(ya<y); assert(za>=0); assert(za<z);  assert(wa>=0); assert(wa<w); data[xa + ya*x + za*x*y + wa*x*y*z] = v; }
    ...
    void print_one_zero(std::string, std::string, FILE*);

    void set_element_name(std::string);  
    void get_element_name(std::string&);
    std::string get_consensus(double, double, double);
