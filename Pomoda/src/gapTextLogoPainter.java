 ...
 */
public class gapTextLogoPainter implements LogoPainter {
  /**
  ...
  public void paintLogo(LogoContext ctxt) {
    ...
    for(Iterator i = info.iterator(); i.hasNext();) {
      ...
      {
      GlyphVector gv = logoFont.createGlyphVector(frc, s.substring(j, j+1).toUpperCase());
      Shape outline = gv.getOutline();
      Rectangle2D oBounds = outline.getBounds2D();

      AffineTransform at = new AffineTransform();
      at.setToTranslation(0.0+(double)j*width/s.length(), base-rv.getValue());
      at.scale(width / oBounds.getWidth()/s.length(), rv.getValue() / oBounds.getHeight());
      at.translate(-oBounds.getMinX(), -oBounds.getMinY());//(1.0-(double)j/s.length())
  ...
  public gapTextLogoPainter() {
    pcs = new PropertyChangeSupport(this);
    logoFont = new Font("Tahoma", Font.PLAIN, 10);//
  }
