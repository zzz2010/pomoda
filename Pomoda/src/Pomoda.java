...
//	    System.exit(1);
	...
	{
		int masklen=0;
		double log_thresh=motif.getThresh(0.99, (double)SearchEngine.getSeqNum()/SearchEngine.TotalLen/2, background);
	...
	public ArrayList<PWM>	 getSeedMotifs() {
		...
		for (int i = 0; i < loopnum; i++) {
			...
			LocList.addAll(SearchEngine2.KmerHitList.get(pattern));
			if(!pattern.equalsIgnoreCase(common.getReverseCompletementString(pattern)))
				LocList.addAll(SearchEngine2.KmerHitList.get(common.getReverseCompletementString(pattern)));
			...
			if((2*SearchEngine2.TotalLen*Math.exp(logprob_bg))>LocList.size())
				continue;
			double score=0;
...
//		}
	...
	public PWM Relax_Seed_3(PWM motif)
	{
		...
		double Prior_EZ= (double)SearchEngine2.getSeqNum()/(SearchEngine2.getTotalLength()/motif.core_motiflen);
		...
		double [] peakrank=new double[num_priorbin];
		int truecount=0;
		while(iter.hasNext())
		{
			FastaLocation currloc=iter.next();
			...
			if(Character.isLowerCase( site.charAt(site.length()/2))&&Math.abs(currloc.getMin()-lastpos)>site.length())//
			{
				truecount++;
...
//		motifBG.conditionProb.put("T", 0.25);
        ...
        Prior_EZ=(double)SearchEngine2.getSeqNum()*0.5/SearchEngine2.TotalLen;//motif.inst_coverage/Falocs.size();//1-SearchEngine.TotalLen*prior_fp/Falocs.size();
...
//		BGsearch.build_index("E:/eclipse/data/promoter_bg.fa");
		PWM bgmodel=background.kmerPWMBG.get(motif.Consensus(true));
...
//			PWM bgmodel=new PWM(allsites);
		if(bgmodel!=null)
			for (int i = 0; i < bgmodel.m_matrix.length; i++) {
				for (int j = 0; j < 4; j++) {
					single_logprob_bg_[i][j]=bgmodel.log_matrix[i][j];
				}
...
//		}
		...
		if(bgmodel!=null)
		{
		bgmodel.head=motif.head;
		bgmodel.tail=motif.tail;
		}
...
//		}
		//update the loglik matrix
				...
				while(iter2.hasNext())
				{
					...
					if(currloc.ReverseStrand)
					{
						site=SearchEngine2.getSite(currloc.getSeqId(), currloc.getSeqPos()-(motif.columns()-seedlen)/2, motif.columns());
						site=common.getReverseCompletementString(site);
					}
					else
						site=SearchEngine2.getSite(currloc.getSeqId(),currloc.getSeqPos()-(motif.columns()-seedlen)/2, motif.columns());
...
//						continue;
					...
					double logprob_BG=0;
					if(background.EnablePWMBG)
						logprob_BG=bgmodel.scoreWeightMatrix(motifsite);
					else
						logprob_BG=motifBG.Get_LOGPROB(site.substring(motif.head,motif.head+motiflen))-bgseed_ignore;
		...
		if(motif.core_motiflen==seedlen)
		{
			...
			for (int k = 0; k < optimzer.colgoodness.length; k++) {
				int extralen=0;
				if(k<motif.head)
				...
				if((extralen+motif.core_motiflen)>maxmotiflen)
					continue;
				if(maxlh<optimzer.colgoodness[k]&&optimzer.colgoodness[k]!=0)
				{
					maxlh=optimzer.colgoodness[k];
					bestCol=k;
					num_col_cand++;
				}
			}
		...
		else
		{
			...
			for (int i = 0; i < motif.columns(); i++) {
				...
				for(Double key: orderSym.descendingKeySet()) {
					...
					temploglik+=count_matrix[i][symid]*Math.exp(logdiff); //key*(1+boundaryLoss);
...
//			}
	...
	double sumLLR(PWM motif)
	{
		...
		//SearchEngine2.EnableBackground(background);
		log_thresh=motif.getThresh(0.99, SearchEngine.SeqNum/SearchEngine.TotalLen, background);
		SearchThread.bestonly=true;
		LinkedList<FastaLocation> Falocs=SearchEngine2.searchPattern(motif, log_thresh);
		SearchThread.bestonly=false;
		Iterator<FastaLocation> iter2=Falocs.iterator();
...
//		}
	...
	double sumLLR(LinkedList<FastaLocation> LocList,double pwmloglik,double bgloglik)
	{
       ...
       Iterator<FastaLocation> iter = LocList.iterator();
		...
		if(OOPS)
		{
			loglik_seq=new double[SearchEngine.getSeqNum()+1];
		}
		while(iter.hasNext())
		{
			FastaLocation currloc=iter.next();
...
//		motifFinder.SearchEngine2.EnableBackground(motifFinder.background);
		 ...
		 ArrayList<AUCComputeThread> threadpool=new ArrayList<AUCComputeThread>(seedPWMs.size());
			...
			{
				BGSearch=new LinearEngine(4);
				BGSearch.build_index(motifFinder.ctrlFasta);
			}
